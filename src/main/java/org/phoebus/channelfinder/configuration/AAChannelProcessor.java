...
public class AAChannelProcessor implements ChannelProcessor {
  ...
  private volatile Map<String, List<String>> cachedPolicies = Map.of();
  ...
  private void refreshPolicies() {
    ...
    logger.log(Level.INFO, "Refreshing AA policy cache for {0} archivers.", aaURLs.size());
    Map<String, List<String>> updated =
        aaURLs.entrySet().stream()
            .filter(e -> !StringUtils.isEmpty(e.getValue()))
            .collect(
                Collectors.toMap(
                    Map.Entry::getKey, e -> archiverService.getAAPolicies(e.getValue())));
    cachedPolicies = Collections.unmodifiableMap(updated);
    lastPolicyRefresh = Instant.now();
    logger.log(
        Level.INFO,
        "AA policy cache refreshed: {0}",
        cachedPolicies.entrySet().stream()
            .map(e -> e.getKey() + "=" + e.getValue().size() + " policies")
  ...
  private Map<String, ArchiverInfo> getArchiversInfoFromCache() {
    ...
    return aaURLs.entrySet().stream()
        ...
        .collect(
            Collectors.toMap(
                Map.Entry::getKey,
                e -> new ArchiverInfo(e.getKey(), e.getValue(), snapshot.get(e.getKey()))));
  }
}
